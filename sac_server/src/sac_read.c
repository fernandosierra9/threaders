...
int sac_server_read(const char *path, char** buf, size_t size, off_t offset) {
	...
	char *data_block;
	size_t tam = size;
	*buf = malloc(sizeof(char) * node->file_size);
	char *buf2;
	if (nodo == -1) return -ENOENT;
...
/* 	if(node->file_size <= offset){
	...
	if (node->file_size <= (offset+size)){
		tam = size = ((node->file_size)-(offset));
		//sac_server_logger_info("Fuse leyendo posicion mayor o igual que el tamanio archivo");
		//sac_server_logger_info("Fuse intenta leer una posicion mayor o igual que el tamanio de archivo. Se retornaran %d bytes. File: %s, Size: %d", path, node->file_size);
	}
	...
	for (bloque_punteros = 0; bloque_punteros < BLOCK_INDIRECT; bloque_punteros++){
		...
		for (num_bloque_datos = 0; num_bloque_datos < 1024; num_bloque_datos++){
			sac_server_logger_info("SEGUNDO IN");
			// Chequea el offset y lo acomoda para leer lo que realmente necesita
			...
			sac_server_logger_info("DATA BLOCK in function: %s", data_block);
			...
			if (tam < BLOCK_SIZE){
				sac_server_logger_info("DATA BLOCK in function: %s", data_block);
				sac_server_logger_info("tam: %d", tam);
				memcpy(*buf, data_block, tam);
				memcpy(buf2, data_block, tam);
				//*buf = &(*buf[tam]);
				buf2 = &(buf2[tam]);
				tam = 0;
				break;
				...
				memcpy(buf2, data_block, BLOCK_SIZE);
				tam -= BLOCK_SIZE;
				//*buf = &(*buf[BLOCK_SIZE]);
				buf2 = &(buf2[BLOCK_SIZE]);
				if (tam == 0) break;
			}
	...
	res = size;
	finalizar:
	sac_server_logger_info("BUF2 in function: %s", buf2);
