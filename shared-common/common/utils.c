...
void utils_serialize_and_send(int socket, int protocol, void* package_send) {
	...
	case OPEN: {
		...
		utils_package_add(package, ((t_open*) package_send)->pathname, strlen(((t_open*) package_send)->pathname)+1);
		utils_package_send_to(package,socket);
		utils_package_destroy(package);
		break;
	}
	...
	case CREATE_DIR: {
		...
		utils_package_add(package, ((t_create_directory*) package_send)->pathname, strlen(((t_create_directory*) package_send)->pathname)+1);
		utils_package_send_to(package,socket);
		utils_package_destroy(package);
		break;
	}
